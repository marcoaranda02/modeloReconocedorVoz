...
WHISPER_MODEL_ID       = "openai/whisper-small"   # pequeño y suficiente para una sola palabra
SAMPLE_RATE_MIC        = 48000
SAMPLE_RATE_MODEL      = 16000
SAMPLE_RATE_WHISPER    = 16000                    # Whisper siempre espera 16kHz
DURACION_GRAB          = 2.0
TARGET_SEC             = 4.0
DEVICE_ID              = 2
UMBRAL_DETECTAR_SONIDO = 0.002

# Variantes aceptadas de "presente" (Whisper puede transcribir con acento o sin él)
PALABRAS_CLAVE = {"presente", "presento", "présente", "present"}
...
def validar_biometria_dsp(audio_np, sr):
    ...
    mfcc_var  = np.mean(np.var(librosa.feature.delta(mfcc), axis=1))
...
def preparar_espectrograma(audio_np):
    ...
    wav_16k = T.Resample(orig_freq=SAMPLE_RATE_MIC, new_freq=SAMPLE_RATE_MODEL)(wav)
    pico    = wav_16k.abs().max()
    if pico > 0:
        wav_16k = wav_16k / pico * 0.95
    mel_db  = T.AmplitudeToDB()(
        T.MelSpectrogram(sample_rate=SAMPLE_RATE_MODEL, n_mels=64, n_fft=1024, hop_length=512)(wav_16k)
...
def procesar_para_ecapa(audio_numpy):
    ...
    target = int(TARGET_SEC * SAMPLE_RATE_MODEL)
    while wav.shape[1] < target:
...
def preparar_audio_whisper(audio_np, sr_origen):
    """
    Convierte el audio a 16kHz mono float32 normalizado,
    listo para el processor de Whisper.
    """
    wav = torch.from_numpy(audio_np).unsqueeze(0)
    if sr_origen != SAMPLE_RATE_WHISPER:
        wav = T.Resample(orig_freq=sr_origen, new_freq=SAMPLE_RATE_WHISPER)(wav)
...
def transcribir(audio_np_16k, processor, whisper_model, device):
    """
    Transcribe el audio con Whisper.
    Forzamos el idioma a español para evitar ambigüedades.
    Devuelve el texto en minúsculas sin puntuación.
    """
    inputs = processor(
        audio_np_16k,
        sampling_rate=SAMPLE_RATE_WHISPER,
        return_tensors="pt"
    ).input_features.to(device)

    # forced_decoder_ids fuerza el idioma a español
    ...
    texto = processor.batch_decode(predicted_ids, skip_special_tokens=True)[0]
    # Limpiar: minúsculas, sin puntuación, sin espacios extra
    texto_limpio = texto.lower().strip().replace(".", "").replace(",", "").replace("¡", "").replace("!", "")
    return texto_limpio


def verificar_palabra_clave(texto_transcrito):
    """
    Devuelve True si alguna palabra clave está en la transcripción.
    Acepta variantes y transcripciones parciales.
    """
    palabras = set(texto_transcrito.split())
    return bool(palabras & PALABRAS_CLAVE)
...
def main():
    ...
    # ── Cargar Whisper ───────────────────────────────────────
    print(f"⏳ Cargando Whisper ({WHISPER_MODEL_ID})... (primera vez descarga ~500MB)")
    ...
    print("\n" + "═"*50)
    print("  PIPELINE: DSP → Liveness → Whisper → ECAPA")
    print("═"*50)
    ...
    while True:
        ...
        with torch.no_grad():
            spec   = preparar_espectrograma(audio_np).to(device)
        ...
        # ── CAPA 3: Whisper — verificación semántica ──────────
        audio_whisper = preparar_audio_whisper(audio_np, sr_origen=SAMPLE_RATE_MIC)
        texto         = transcribir(audio_whisper, whisper_processor, whisper_model, device)
        print(f"📝 Whisper transcribió: \"{texto}\"")

        if not verificar_palabra_clave(texto):
            print(f"🛡️  [BLOQUEADO · WHISPER] No se detectó 'presente' en el audio.")
            print(f"    Palabras aceptadas: {PALABRAS_CLAVE}")
            continue

        print(f"✅ Palabra clave detectada.")

        # ── CAPA 4: Reconocimiento ECAPA ──────────────────────
